...
const AssistantList = () => {
  ...
  const fetchassistants = async () => {
    setLoading(true);
    try {
      const res = await axiosAuth.get("/assistants");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setassistants(assistants.filter((t) => t._id !== id));
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-8 w-full mx-auto bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-gray-800">assistants List</h2>
      </div>

      {loading && <div className="mb-4 text-gray-500">Loading assistants...</div>}
